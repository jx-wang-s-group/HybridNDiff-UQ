...
def grid(nNodes = 100):
    ...
    # ----------------------------- Grid --------------------------------------------
    grid_x = jnp.linspace(-1,1,nNodes)
    grid_y = jnp.linspace(-1,1,nNodes)
    grid_x, grid_y = jnp.meshgrid(grid_x, grid_y, indexing='ij')
    grid_dx = jnp.concatenate((grid_x[1:2,:]-grid_x[0:1,:], 0.5*(grid_x[2:,:]-grid_x[:-2,:]), grid_x[-1:,:]-grid_x[-2:-1,:]), axis=0)
    grid_dy = jnp.concatenate((grid_y[:,1:2]-grid_y[:,0:1], 0.5*(grid_y[:,2:]-grid_y[:,:-2]), grid_y[:,-1:]-grid_y[:,-2:-1]), axis=1)
    Grid = {'nNodes':nNodes,'grid_x':grid_x, 'grid_y':grid_y, 'grid_dx':grid_dx, 'grid_dy':grid_dy}
    return Grid
